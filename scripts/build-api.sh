...
screen -DRRS controller -X stuff  "python3 -m startchat.serve.controller \r"

screen -DRRS worker-d0 -X stuff  "CUDA_VISIBLE_DEVICES=0 python3 -m startchat.serve.model_worker --model-path $MODEL_PATH  --conv-template one_shot --limit-worker-concurrency 1  \r"
screen -DRRS worker-d1 -X stuff  "CUDA_VISIBLE_DEVICES=1 python3 -m startchat.serve.model_worker --model-path $MODEL_PATH --port 21003 --worker-address http://localhost:21003  --conv-template one_shot --limit-worker-concurrency 1  \r"

screen -DRRS api -X stuff  "python3 -m startchat.serve.openai_api_server --host $API_HOST --port $API_PORT_NUMBER \r"
