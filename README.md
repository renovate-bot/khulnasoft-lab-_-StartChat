...
StartChat is an open platform for training, serving, and evaluating large language model based chatbots.
- StartChat powers Chatbot Arena (https://chat.lmsys.org/), serving over 6 million chat requests for 50+ LLMs.
...
StartChat's core features include:
...
1. Clone this repository and navigate to the StartChat folder
```bash
...
| ---  | --- | --- |
| 7B   | `python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5`  | [lmsys/vicuna-7b-v1.5](https://huggingface.co/lmsys/vicuna-7b-v1.5)   |
| 7B-16k   | `python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5-16k`  | [lmsys/vicuna-7b-v1.5-16k](https://huggingface.co/lmsys/vicuna-7b-v1.5-16k)   |
| 13B  | `python3 -m startchat.serve.cli --model-path lmsys/vicuna-13b-v1.5` | [lmsys/vicuna-13b-v1.5](https://huggingface.co/lmsys/vicuna-13b-v1.5) |
| 13B-16k  | `python3 -m startchat.serve.cli --model-path lmsys/vicuna-13b-v1.5-16k` | [lmsys/vicuna-13b-v1.5-16k](https://huggingface.co/lmsys/vicuna-13b-v1.5-16k) |
| 33B  | `python3 -m startchat.serve.cli --model-path lmsys/vicuna-33b-v1.3` | [lmsys/vicuna-33b-v1.3](https://huggingface.co/lmsys/vicuna-33b-v1.3) |
...
| Size | Chat Command | Hugging Face Repo |
| ---  | --- | --- |
| 7B   | `python3 -m startchat.serve.cli --model-path lmsys/longchat-7b-32k-v1.5`  | [lmsys/longchat-7b-32k](https://huggingface.co/lmsys/longchat-7b-32k-v1.5)   |

### StartChat-T5
You can use the commands below to chat with StartChat-T5. It will automatically download the weights from Hugging Face repos.

| Size | Chat Command | Hugging Face Repo |
| ---  | --- | --- |
| 3B   | `python3 -m startchat.serve.cli --model-path lmsys/startchat-t5-3b-v1.0`  | [lmsys/startchat-t5-3b-v1.0](https://huggingface.co/lmsys/startchat-t5-3b-v1.0) |
...
#### Supported Models
StartChat supports a wide range of models, including
LLama 2, Vicuna, Alpaca, Baize, ChatGLM, Dolly, Falcon, StartChat-T5, GPT4ALL, Guanaco, MTP, OpenAssistant, OpenChat, RedPajama, StableLM, WizardLM, xDAN-AI and more.
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --num-gpus 2 --max-gpu-memory 8GiB
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device cpu
```

Use Intel AI Accelerator AVX512_BF16/AMX to accelerate CPU inference.
```
CPU_ISA=amx python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device cpu
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device mps --load-8bit
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device xpu
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device npu
```
...
```
python3 -m startchat.serve.cli --model-path lmsys/vicuna-7b-v1.5 --load-8bit
```
...
- For AMD GPU users, please install ROCm and [the ROCm version of PyTorch](https://pytorch.org/get-started/locally/) before you install StartChat. See also this [post](https://github.com/khulnasoft-lab/StartChat/issues/104#issuecomment-1613791563).
- StartChat supports ExLlama V2. See [docs/exllama_v2.md](/docs/exllama_v2.md).
- StartChat supports GPTQ 4bit inference with [GPTQ-for-LLaMa](https://github.com/qwopqwop200/GPTQ-for-LLaMa). See [docs/gptq.md](/docs/gptq.md).
- StartChat supports AWQ 4bit inference with [mit-han-lab/llm-awq](https://github.com/mit-han-lab/llm-awq). See [docs/awq.md](/docs/awq.md).
...
```bash
python3 -m startchat.serve.test_message --model-name vicuna-7b-v1.5
```
You will see a short output.

#### Launch the Gradio web server
```bash
python3 -m startchat.serve.gradio_web_server
```
...
```
# worker 0
CUDA_VISIBLE_DEVICES=0 python3 -m startchat.serve.model_worker --model-path lmsys/vicuna-7b-v1.5 --controller http://localhost:21001 --port 31000 --worker http://localhost:31000
# worker 1
CUDA_VISIBLE_DEVICES=1 python3 -m startchat.serve.model_worker --model-path lmsys/startchat-t5-3b-v1.0 --controller http://localhost:21001 --port 31001 --worker http://localhost:31001
```
- You can also launch a multi-tab gradio server, which includes the Chatbot Arena tabs.
```bash
python3 -m startchat.serve.gradio_web_server_multi
```
...
```bash
export STARTCHAT_USE_MODELSCOPE=True
```
...
The REST API is capable of being executed from Google Colab free tier, as demonstrated in the [StartChat_API_GoogleColab.ipynb](https://github.com/khulnasoft-lab/StartChat/blob/main/playground/StartChat_API_GoogleColab.ipynb) notebook, available in our repository.
See [docs/openai_api.md](docs/openai_api.md).

### Hugging Face Generation APIs
See [startchat/serve/huggingface_api.py](startchat/serve/huggingface_api.py).
...
MT-bench is the new recommended way to benchmark your models. If you are still looking for the old 80 questions used in the vicuna blog post, please go to [vicuna-blog-eval](https://github.com/khulnasoft-lab/vicuna-blog-eval).
...
```bash
torchrun --nproc_per_node=4 --master_port=20001 startchat/train/train_mem.py \
...
Tips:
- If you are using V100 which is not supported by FlashAttention, you can use the [memory-efficient attention](https://arxiv.org/abs/2112.05682) implemented in [xFormers](https://github.com/facebookresearch/xformers). Install xformers and replace `startchat/train/train_mem.py` above with [startchat/train/train_xformers.py](startchat/train/train_xformers.py).
...
More instructions to train other models (e.g., StartChat-T5) and use LoRA are in [docs/training.md](docs/training.md).
